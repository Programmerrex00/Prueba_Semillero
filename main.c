...
void menuMostrar(){
	...
	if (numeroProductos == 0) {
		...
		if (strcmp(respuesta, "S") == 0 || strcmp(respuesta, "s") == 0) {
			...
			getchar(); //se usa para que el usuario ingrese un caracter para continuar
		}
	}
}
...
char eliminarProducto(int codigoProducto){   //Tampoco lo usare
...
char eliminacionFisica() ///NO lo voy a usar
{
	FILE *archivo;
	FILE *temporal;
	Producto producto;
